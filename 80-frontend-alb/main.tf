...
resource "aws_route53_record" "ingress_alb" {
  zone_id = var.zone_id
  name    = "*.${var.domain_name}" # roboshop-dev.deepthi.cloud
  type    = "A"
  allow_overwrite = true

  alias {
    # These are ALB Details,not our domain details.
    name                   = aws_lb.ingress_alb.dns_name # aws_elb.ingress_alb.dns_name comes from your ALB resource.Public DNS name AWS assigns to your Load Balancer.
